...
function Account(){
    ...
    $usersCount = count(explode("\n",file_get_contents("users1")))-1;
    
    if ($usersCount <= 4) {
        
        bot('sendMessage',['chat_id' => $id, 'text' => 'Starting Turbo ... 1']);
        ...
        bot('sendMessage',['chat_id' => $id, 'text' => 'Starting Checker ... 1']);
...
if(json_decode(file_get_contents('mode.json'),true)['number1']['method'] == 'account'){
    Account();
} else {
    Channel();
}
