...
function Account(){
    ...
    $usersCount = count(explode("\n",file_get_contents("users2")))-1;
    
    if ($usersCount <= 4) {
        
        bot('sendMessage',['chat_id' => $id, 'text' => 'Starting Turbo ... 2']);
        ...
        bot('sendMessage',['chat_id' => $id, 'text' => 'Starting Checker ... 2']);
...
if(json_decode(file_get_contents('mode.json'),true)['number2']['method'] == 'account'){
    Account();
} else {
    Channel();
}
