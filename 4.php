...
function Account(){
    ...
    $usersCount = count(explode("\n",file_get_contents("users4")))-1;
    
    if ($usersCount <= 4) {
        
        bot('sendMessage',['chat_id' => $id, 'text' => 'Starting Turbo ... 4']);
        ...
        bot('sendMessage',['chat_id' => $id, 'text' => 'Starting Checker ... 4']);
...
if(json_decode(file_get_contents('mode.json'),true)['number4']['method'] == 'account'){
    Account();
} else {
    Channel();
}
