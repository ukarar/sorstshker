...
function Account(){
    ...
    $usersCount = count(explode("\n",file_get_contents("users3")))-1;
    
    if ($usersCount <= 4) {
        
        bot('sendMessage',['chat_id' => $id, 'text' => 'Starting Turbo ... 3']);
        ...
        bot('sendMessage',['chat_id' => $id, 'text' => 'Starting Checker ... 3']);
...
if(json_decode(file_get_contents('mode.json'),true)['number3']['method'] == 'account'){
    Account();
} else {
    Channel();
}
